...
################################################################################
################################################################################
# Extra example with Identification Problem
################################################################################
################################################################################

# Set options to impose restriction.
opt1 <- opt
# Same restriction as above to thest the hypothesis 
# that the interest-rate is long-run exogenous.
opt1$R_Alpha <- matrix(c(0, 1, 0), nrow = 1, ncol = 3)
# Include a restriction that the first cointegrating vector
# has element equal to 1 on the support for the liberal party.
opt1$R_Beta <- matrix(c(1, 0, 0), nrow = 1, ncol = 3)
opt1$r_Beta <- matrix(2, nrow = 1, ncol = 1)

# Estimate model and store in an FCVARmodel object.
m1r6 <- FCVARestn(x1, k, r, opt1)

# Test the null of m1r6 against the alternative m1.
Halphabeta1 <- FCVARhypoTest(m1, m1r6)


#------------------------------------------------------------
# Two more examples m1r7 and m1r8
# with two ways of testing beta_1 = k*beta_2:
# one with numbers and one with proportions.
#------------------------------------------------------------

# Specify the numbers exactly.
opt1 <- opt
opt1$R_Alpha <- matrix(c(0, 1, 0), nrow = 1, ncol = 3)
opt1$R_Beta <- matrix(c(1, 0, 0, 
                        0, 1, 0), nrow = 2, ncol = 3, byrow = TRUE)
opt1$r_Beta <- matrix(c(2, 0.5), nrow = 2, ncol = 1)
m1r7 <- FCVARestn(x1, k, r, opt1)

Halphabeta2 <- FCVARhypoTest(m1, m1r7)


# Specify only the ratio between numbers.
# Specify the numbers exactly.
opt1 <- opt
opt1$R_Alpha <- matrix(c(0, 1, 0), nrow = 1, ncol = 3)
opt1$R_Beta <- matrix(c(1, -4, 0), nrow = 1, ncol = 3)
# opt1$r_Beta <- matrix(c(2, 0.2), nrow = 2, ncol = 1)
m1r8 <- FCVARestn(x1, k, r, opt1)

Halphabeta3 <- FCVARhypoTest(m1, m1r8)
# Same test result as Halphabeta2.
...
# Generate quantiles of the asymptotic distribution
...
# Store the quantiles.
LR_Rnk_pvals <- rep(NA, LR_Rk_num)
for (i in 1:LR_Rk_num) {
  print(i)
  LR_Rnk_pvals[i] <- fracdist::fracdist_values(iq = p - r,
                                            iscon = consT,
                                            bb = m1$coeffs$db[2],
                                            stat = LR_Rnk_range[i])
...
# fig_ext <- 'png'
# out_file_path <- sprintf('%s/RankDensity_bw045.%s', fig_dir, fig_ext)
# png(out_file_path)
plot(LR_Rnk_density,
     main = c('Bootstrap Density with Density of Asymptotic Distribution',
